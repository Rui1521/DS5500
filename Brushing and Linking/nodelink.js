...
function setupNodelink(graph)
{
    ...
    dispatch.on("stationHovered.node", function (station) {
        if (station) {
            node.attr("fill", "#ccc")
                .filter(function (d) {
...
dispatch.on("stationHovered", function (station) {
    if (station) {
        d3.select('#station-name').html(station.name)
    } else {
        d3.select('#station-name').html('')
    }
})
